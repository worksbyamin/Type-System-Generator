...
import { Columns2, Monitor, Smartphone, PanelLeftOpen, Menu, Type, Sun, Moon } from 'lucide-react';
...
export const WorkspaceToolbar: React.FC<WorkspaceToolbarProps> = ({
  ...
  return (
    <div className="min-h-14 py-2 px-3 sm:px-4 md:px-6 bg-neutral-50 dark:bg-[#131315]/90 backdrop-blur border-b border-black/10 dark:border-white/10 flex flex-wrap items-center justify-between shrink-0 gap-2 sm:gap-3">
      {/* LEFT: Mode Toggle (Type vs Simulation) & Sidebar Restore Button */}
      <div className="flex items-center gap-2">
        {/* Mobile Menu Toggle & Logo */}
        <div className="flex md:hidden items-center gap-2 pr-3 border-r border-black/10 dark:border-white/10 mr-1">
          <button
            onClick={() => onUpdateAppConfig({ sidebarCollapsed: false })}
            className="p-1.5 -ml-1 rounded-lg text-neutral-600 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
            title="Open Menu"
      ...
      <div className="flex justify-center order-3 sm:order-2 w-full sm:w-auto">
        {previewMode === 'simulation' && (
          <div className="flex bg-neutral-200 dark:bg-[#232326] p-1 rounded-xl gap-1 shadow-sm">
            <button
      ...
      <div className="hidden md:flex items-center gap-1.5 order-2 sm:order-3">
        {/* Theme Toggle */}
        <div className="flex bg-neutral-200 dark:bg-[#232326] p-1 rounded-xl gap-1 mr-2">
          <button
            onClick={() => onUpdateAppConfig({ uiTheme: 'light' })}
            title="Light Mode UI"
            aria-label="Light Mode UI"
            className={`p-1.5 rounded-lg text-xs transition-all flex items-center justify-center cursor-pointer ${
              appConfig.uiTheme !== 'dark'
                ? 'bg-white text-black font-semibold shadow-sm'
                : 'text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'
            }`}
          >
          ...
          <button
            onClick={() => onUpdateAppConfig({ uiTheme: 'dark' })}
            title="Dark Mode UI"
            aria-label="Dark Mode UI"
            className={`p-1.5 rounded-lg text-xs transition-all flex items-center justify-center cursor-pointer ${
              appConfig.uiTheme === 'dark'
                ? 'bg-white text-black font-semibold shadow-sm'
                : 'text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'
