...
export function loadGoogleFont(fontFamily: string): void {
  ...
  try {
    ...
    if (axes.length > 0) {
      // It's a known variable font. Let's request just ital and wght for safety, 
      // as opsz can cause 400 errors if not supported by the specific font.
      // But actually Google Fonts allows requesting variable wght range if it's a variable font.
      ...
      // For non-variable fonts, requesting a range like 100..900 can cause a 400 Bad Request.
      // We explicitly request discrete weights instead.
      // Google Fonts ignores weights that the font doesn't support, but requesting a range for a non-variable font fails.
      link.href = `https://fonts.googleapis.com/css2?family=${encodeURIComponent(
        fontFamily
      )}:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;1,300;1,400;1,500;1,600;1,700;1,800&display=swap`;
    }
